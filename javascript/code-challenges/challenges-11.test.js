...
function transformToLis(obj) {
  return Object.entries(obj).map(keyValPair => `<li>${keyValPair.join(': ')}</li>`);
}
...
const totalSum = (input) => {
  let flatArr = input.flat();
  let sum = flatArr.reduce((acc, curVal) => {
    acc += curVal;
    return acc;
  }, 0)
  return sum;
...
const divisibleByFiveTwoToThePower = (input) => {
  let filterArr = input.map(arr => {
    return arr.filter(elem =>
      typeof elem === 'number' && elem % 5 === 0
    );
  })
  let finalArr = filterArr.map(numArr => {
    return numArr.map(num => Math.pow(2, num));
  })
  return finalArr;
...
let findMaleAndFemale = (data) => {
  let filteredObjArr = data.filter(obj => {
    return obj.gender === 'male' || obj.gender === 'female'
  });
  let objNamesArr = filteredObjArr.map(obj => obj.name);
  let nameString = objNamesArr.join(' and ');
  return nameString;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
let findShortest = (data) => {
  let heights = data.map(obj => +obj.height);
  let min = Math.min(...heights);
  let nameObj = data.filter(obj => +obj.height === min)[0];
